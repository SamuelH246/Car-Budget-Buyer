...
# Ask for budget
print(f"Alright {name}, Let's find a car you can afford!")
...
while True:
    ...
    if not affordable_cars:
        print("You can't afford anymore cars or stock is out. Time to checkout.")
        break
    ...
    selection_number = int_check("Which car would you like to purchase?, or type '0' to proceed to cart: ", 0, len(affordable_cars))
    ...
    # Asks how many user wants to buy (option to return) + calculates total price
    print(f"\nYou can add up to {max_can_buy} of {selected_item['item']} to your cart.")
    quantity = int_check(f"How many {selected_item['item']} would you like to add? (or enter 0 to return):\n", 0, max_can_buy)
    if quantity == 0:
        continue
...
# Checkout
if not cart:
    print("You haven’t selected anything for your cart, the program will exit.")
    print("Thank you for using Car Budget Buyer!")
    quit()
